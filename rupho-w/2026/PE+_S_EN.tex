...
The amount of a substance in a mixture is defined as: \[\nu=\dfrac{m}{\mu},\] where $m$ is the mass of the substance, and $\mu$ is its molar mass. 

Let us write the formula for the molar concentration of water: \[n_w=1-n_g=1-\dfrac{\nu_g}{\nu_g+\nu_w},\] from which we obtain:
...
Let us recalculate the data to find the dependence $\ln\left(\dfrac{\eta}{1~\mathrm{Pa}\cdot \mathrm{s}}\right)(n_w)$.
...
Let us plot the resulting dependence in the range $n_w \in [0, 0.2]$.
...
Let us plot the resulting points on the graph $\ln\left|\dfrac{\xi \exp(-kn_w)}{\eta}-1\right|(n_w)$.
...
For $n_w>0.2$, $\dfrac{\xi \exp(-kn_w)}{\eta}>1$ holds true, so $a>0$. By drawing the approximating straight line, we find: \[b=5.66,\]\[\ln a=-3.21,\] from which we obtain:
...
Let us calculate the values using the theoretical dependence $\eta(n_w)$:
...
Let us plot the obtained dependencies $\eta(n_w)$:

By drawing smoothed curves, we conclude:

\par\smallskip\noindent\textbf{Answer:} The equation is applicable over the entire range ($n_w \in [0, 1]$).
\begin{figure}[H]
  \centering
...
Having determined the parameters of the approximating straight line, we obtain a system of equations: \[\begin{cases}-\dfrac{1}{D}=-0.059\\ \dfrac{1}{DT_0}=6.38\cdot 10^{-4}~\mathrm{K}^{-1},\end{cases}\] from which it follows:

\par\smallskip\noindent\textbf{Answer:} \[T_0=92.5~\mathrm{K}\]\[D=16.9\]
\begin{figure}[H]
  \centering
...
Similar to the previous section, we substitute the value $T=25~^\circ\mathrm{C}=298~\mathrm{K}$ into the equation obtained in part B, from which it follows:

\par\smallskip\noindent\textbf{Answer:} $\eta_t=6.8~\mathrm{mPa}\cdot \mathrm{s}$
\end{solcard}

\begin{solcard}{C3}{Compare your results. If there are discrepancies, explain the reason for them.}{0.20}
\par\smallskip\noindent\textbf{Answer:} Considering the large measurement range, the results can be considered consistent. The slight difference is most likely due to inaccurate temperature measurements, since the dependence of viscosity on temperature is very strong.
\end{solcard}
...
The straight line must pass through the origin $(0, 0)$. By fitting it to the four initial data points, we find $k$: \[k=0.674\cdot 10^{-6}~\dfrac{\mathrm{m}^3}{\mathrm{s}^2}\] Now knowing $k$, we can restore the dependence $v_2(\eta)$:
...
\begin{solcard}{D2}{Construct a graph of $v_1(\eta)$ in the range from $\eta_1=10^{-3}~\mathrm{Pa}\cdot \mathrm{s}$ to $\eta_2=10^{3}~\mathrm{Pa}\cdot \mathrm{s}$ in the coordinates that are most convenient for graphical analysis. All points plotted on the graph must be tabulated on the answer sheet.}{1.50}

At high viscosities, the velocities in hydrophobic and hydrophilic capillaries are equal. At $\eta=1.400~\mathrm{Pa}\cdot \mathrm{s}$, $v_1>v_2$; therefore, we assume that $v_1=v_2$ for $\eta>\eta_{\text{max}}=1.400~\mathrm{Pa}\cdot \mathrm{s}$. Let us enter the points into the table and construct a graph in logarithmic coordinates, since the values of the quantities under study vary by several orders of magnitude.

\begin{table}[H]
\centering
\begin{tabular}{|c|c|c|c|c|c|c|c|}
\hline
$\eta,~\mathrm{mPa}\cdot\mathrm{s}$ & $v_1,~\mathrm{\mu m}/\mathrm{s}$ & $v_2,~\mathrm{mm}/\mathrm{s}$ & $n_w$ & $\rho, ~\dfrac{\mathrm{kg}}{\mathrm{m}^3}$ & $\rho/\eta, ~\dfrac{\mathrm{s}}{\mathrm{m}^2}\cdot 10^3$ & $\ln\dfrac{v_2}{1~\mathrm{mm}/\mathrm{s}}$ & $\ln\dfrac{\eta}{1~\mathrm{Pa}\cdot \mathrm{s}}$ \\
\hline
...
\begin{tabular}{|c|c|c|c|c|c|c|c|}
\hline
$\eta,~\mathrm{Pa}\cdot\mathrm{s}$ & $v_1,~\mathrm{\mu m}/\mathrm{s}$ & $v_2,~\mathrm{mm}/\mathrm{s}$ & $n_w$ & $\rho, ~\dfrac{\mathrm{kg}}{\mathrm{m}^3}$ & $\rho/\eta, ~\dfrac{\mathrm{s}}{\mathrm{m}^2}$ & $\ln\dfrac{v_1}{1~\mathrm{\mu m}/\mathrm{s}}$ & $\ln\dfrac{\eta}{1~\mathrm{Pa}\cdot \mathrm{s}}$ \\
\hline
...
Let us plot the obtained points on the graph: $\ln\left(\dfrac{v_1}{1~\mathrm{\mu m}/\mathrm{s}}\right)$ versus $\ln\left(\dfrac{\eta}{1~\mathrm{Pa}\cdot \mathrm{s}}\right)$.
...
\par\smallskip\noindent\textbf{Answer:} \[\eta_{\text{max}}=1.400~\mathrm{Pa}\cdot \mathrm{s}\]
\end{solcard}
