...
Consider the following numerical data known: Density of glycerol $\rho_g=1260~kg/м^3$; Density of water $\rho_w=1000~kg/м^3$; Molar mass of glycerol $\mu_g=92~г/mol$; The molar mass of water is $\mu_w=18~г/mol$. In all parts of the problem, you can consider the volume of the mixture to be equal to the sum of the volumes of the components that make up this mixture.

In all parts of the problem, you can consider the volume of the mixture to be equal to the sum of the volumes of the components that make up this mixture.
...
To describe the dependence of viscosity $\eta$ on temperature $T$ the modified equation is often used Andrade$$\textbackslash\{\}eta = A \textbackslash\{\}cdot \textbackslash\{\}exp\textbackslash\{\}left(\textbackslash\{\}frac\{B\}\{T\} + C\textbackslash\{\}right),\textbackslash\{\}tag\{3\}$$где $A$, $B$ и $C$ – некоторые постоянные. Оно хорошо описывает поведение cmеси при больших температурах, однако перестаёт работать при приближении к температуре стеклования. При низких температурах, близких к температуре стеклования, вместо $(1)$ удобно использовать уравнение Вогеля–Таmmана–Фульхера:⟦0⟧где $\eta_0$, $D$ и $T_0$ — некоторые постоянные. В таблице представлена экспериментальная зависимость вязкости cmеси от температуры. Измерения производились со cmесью с объемной концентрацией глицерина $c_g=0.5$.

At low temperatures close to the glass transition temperature, instead of $(1)$ it is convenient to use the Vogel–Tammann–Fulcher equation: \textbackslash\{\}[\textbackslash\{\}eta = \textbackslash\{\}eta_0 \textbackslash\{\}cdot \textbackslash\{\}exp\textbackslash\{\}left(\textbackslash\{\}frac\{DT_0\}\{T - T_0\}\textbackslash\{\}right),\textbackslash\{\}tag\{4\}\textbackslash\{\}]where $\eta_0$, $D$ and $T_0$ are some constants.
...
Let us assume that the value $\eta_0=3.33\cdot 10^{-3}~{Па\cdot с}.$ is known
