...
Let us now take into account the curvature of the Earth. Its radius is $R_\mathrm E=6.4\cdot10^3~\text{k}$. The remaining numerical values are $m=29\cdot1.66\cdot10^{-27}~\text{kg}$, $T=300~\text{K}$, $k_\mathrm B=1.38\cdot10^{-23}~\frac{\text{J}}{\text{K}}$, $g=9.8~\frac{\text{m}}{\text{s}^2}$.
...
The atmosphere of Venus consists of $\mathrm{CO_2}$, which can be considered ideal and for which all the results obtained above can be applied. The atmospheric pressure on the surface of Venus is $p_\mathrm V=93~\text{atm}$ ($1~\text{atm}=1.0\cdot10^5~\text{Pa}$), and the temperature is $T_\mathrm V=740~\text{K}$. The radius of Venus is $R_\mathrm V=6.0\cdot10^3~\text{k}$, the acceleration of gravity on its surface is $g_\mathrm V=8.9~\frac{\text{m}}{\text{s}^2}$, the mass of the molecule is $\mathrm{CO_2}$ $m_\mathrm C=44\cdot1.66\cdot10^{-27}~\text{kg}$. For the refractive index on Venus, the equation is valid: \[n^2=1+\alpha_VN\] with a coefficient $\alpha_\mathrm V=4\pi a_\mathrm C^3$, where $a_\mathrm C=1.4\cdot10^{-10}~\text{m}$ is the effective size of the molecule $\mathrm{CO_2}$.
