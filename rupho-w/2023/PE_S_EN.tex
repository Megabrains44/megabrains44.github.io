...
\title{\textbf{Determination of refractive index --- Solution}}
\author{\normalsize W23 (2023) — English translation}
\date{}
\begin{document}\maketitle\vspace{-0.5em}
\begin{solcard}{A1}{For each $x$, find $n$.}{0.41}
\par\smallskip\noindent\textbf{Answer:} \textbackslash{}textbf{Answer:} See $\bf A6$.
\end{solcard}

\begin{solcard}{A2}{For each $x$, find $\lambda$.}{0.82}
\par\smallskip\noindent\textbf{Answer:} \textbackslash{}textbf{Answer:} See $\bf A6$.
\end{solcard}

\begin{solcard}{A3}{For each $x$, find $\mathcal J_\lambda$ (in arbitrary units per nm). Since for each $x$ we have already calculated $\lambda$, we obtain the dependence $\mathcal J_\lambda(\lambda)$.}{0.82}
\par\smallskip\noindent\textbf{Answer:} \textbackslash{}textbf{Answer:} See $\bf A6$.
\end{solcard}

\begin{solcard}{A4}{Plot a graph of $\mathcal J_\lambda(\lambda)$.}{0.49}
\begin{figure}[H]
  \centering
...
\begin{solcard}{A6}{Find for each $x$ the value $\sigma$. Since for each $x$ we found a $\lambda$, we also obtained the dependence $\lambda(\sigma)$.}{1.23}
