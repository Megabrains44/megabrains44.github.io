...
The equation obtained in step $\mathrm{A1}$ is reduced to the following form: $$13\left(\omega_{i+1}-\cfrac{v}{l}\right)=7\left(\omega_i-\cfrac{v}{l}\right) $$ Introducing variable $\omega'_i=\omega_i-v/l$, obtain: $$13\omega'_{i+1}=7\omega'_i $$ From which: $$\omega'_i=\omega'_1\cdot{\left(\cfrac{7}{13}\right)^{i-1}} $$ Since $\omega_0 =0$, $\omega_1=6v/(13l)$. Then $\omega'_1=-7v/(13l)$, from which : $$\omega'_i=-\cfrac{v}{l}\left(\cfrac{7}{13}\right)^i $$ and finally
\par\smallskip\noindent\textbf{Answer:} $$\omega_i=\cfrac{v}{l}\left(1-\left(\cfrac{7}{13}\right)^i\right) $$
\end{solcard}
...
Let us use the conservation of the velocity component $v_\perp$. Immediately after the impact, the velocity vector of the ball $\vec{u}$ is equal to: $$\vec{u}=\vec{v}+\cfrac{\Delta{\vec{p}}_N}{m} $$ where $\Delta{\vec{p}}_N$ - momentum force reaction, act on ball with side sphere. Since $\Delta{\vec{p}}_N$ direct along line center, from figure find: $$u=v\tan\alpha $$
...
\par\smallskip\noindent\textbf{Answer:} $$k_0=\sqrt{\cfrac{I-ml^2}{2I-ml^2}}=\sqrt{\cfrac{7}{17}} $$
\begin{figure}[H]
  \centering
...
Let us decompose the velocity of the ejected ball into velocity components $v_\parallel$ and $v_\perp$, directed respectively along and perpendicular to the line of centers. The angular momentum of the ball relative to point $O$ is equal to: $$\vec{L}_O=m\big[\vec{v}\times\vec{r}\big]=m\big[\vec{v}_\parallel\times\vec{r}\big]+m\big[\vec{v}_\perp\times\vec{r}\big] $$ On figure denote lever arm velocity $v_\parallel$ and $v_\perp$ relatively axis rotation, correspondingly equal $l\cos\alpha$ and $l\sin\alpha$.

The speed of the ball immediately after the impact will also be decomposed into $u_\parallel$ and $u_\perp$. Let us choose their directions to be the same as for $v_\parallel$ and $v_\perp$, respectively. Let us write down the laws of conservation of energy and angular momentum relative to the axis of rotation: $$\begin{cases} E=\cfrac{I\omega^2_i}{2}+\cfrac{m(v^2_\parallel+v^2_\perp)}{2}=\cfrac{I\omega^2_{i+1}}{2}+\cfrac{m(u^2_\parallel+u^2_\perp)}{2}\\ L=I\omega_i+mv_\parallel l\cos\alpha+mv_\perp l\sin\alpha=I\omega_{i+1}+mu_\parallel l\cos\alpha+mu_\perp l\sin\alpha \end{cases}$$ Since $v_\perp=u_\perp$, and $v_\parallel=v\cos\alpha$, rewrite system more compactly: $$\begin{cases} I\omega^2_i+mv^2\cos^2\alpha=I\omega^2_{i+1}+mu^2_\parallel\\ I\omega_i+mvl\cos^2\alpha=I\omega_{i+1}+mu_{\parallel}l\cos\alpha \end{cases}$$
 $u_\parallel$: $$u^2_\parallel=v^2\cos^2\alpha-\cfrac{I(\omega^2_{i+1}-\omega^2_i)}{m}=\left(v\cos\alpha-\cfrac{I(\omega_{i+1}-\omega_i)}{ml\cos\alpha}\right)^2 $$ Expanding the brackets and canceling the factor $\omega_{i+1}-\omega_i$, we get: $$\cfrac{2v}{l}=\omega_{i+1}\left(1+\cfrac{I}{ml^2\cos^2\alpha}\right)-\omega_i\left(\cfrac{I}{ml^2\cos^2\alpha}-1\right) $$
\par\smallskip\noindent\textbf{Answer:} $$\omega_{i+1}=\cfrac{6v(1-k^2)}{l(13-3k^2)}+\omega_i\cdot\cfrac{7+3k^2}{13-3k^2} $$
\begin{figure}[H]
  \centering
...
\par\smallskip\noindent\textbf{Answer:} $$\omega_1=\cfrac{6v(1-k^2)}{l(13-3k^2)} $$
\end{solcard}
...
Let's introduce the variable $\omega'_i=\omega_i-v/l$ again and get: $$\omega'_{i+1}\left(1+\cfrac{I}{ml^2\cos^2\alpha}\right)=\omega'_i\left(\cfrac{I}{ml^2\cos^2\alpha}-1\right) $$ from which after substitution $I$: $$\omega'_{i+1}=\omega'_i\left(\cfrac{7+3k^2}{13-3k^2}\right)\Rightarrow \omega'_i=\omega'_1\left(\cfrac{7+3k^2}{13-3k^2}\right)^{i-1} $$ For $\omega'_1$ have: $$\omega'_1=\omega_1-\cfrac{v}{l}=-\cfrac{v(7+3k^2)}{l(13-3k^2)} $$ Thus: $$\omega'_i=\cfrac{v}{l}\left(\cfrac{7+3k^2}{13-3k^2}\right)^i $$ and finally:
\par\smallskip\noindent\textbf{Answer:} $$\omega_i=\cfrac{v}{l}\left(1-\left(\cfrac{7+3k^2}{13-3k^2}\right)^i\right) $$
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \[\alpha < \frac{2\varepsilon^2\omega_0^2}{\pi(1+\varepsilon)^2}\]
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \[\beta < \frac{2\varepsilon\omega_0}{\pi(1+\varepsilon)}\]
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \[\gamma < +\infty\]
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \[\omega^\uparrow=\omega_0-\frac{\pi\beta}2\left(\varepsilon^{-1}-1\right)\]
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \[\overline\omega_2=\frac{\pi\beta}{\ln\left(2\varepsilon\omega_0-\pi\beta(1-\varepsilon)\right)-\ln\left(2\varepsilon\omega_0-\pi\beta(1+\varepsilon)\right)}\]
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \[\omega^\uparrow=\frac{2\omega_0\varepsilon}{1-e^{-\pi\gamma}+\varepsilon\left(1+e^{-\pi\gamma}\right)}\]
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \[\overline\omega_3=\frac{\pi\varepsilon\gamma\omega_0}{2\operatorname{sh}(\pi\gamma/2)\left[\operatorname{sh}(\pi\gamma/2)+\varepsilon\operatorname{ch}(\pi\gamma/2)\right]}\]
\end{solcard}
