...
\par\smallskip\noindent\textbf{Answer:} \[\lambda=\sqrt{\dfrac{\varepsilon \varepsilon_0 k_BT}{\displaystyle\sum\limits_in_{i0}q_i^2}}\]
\end{solcard}

\begin{solcard}{A2}{Assume that the extracellular solution is $\mathrm{NaCl}$ with a concentration of $440~\mathrm{mmol}/\mathrm{L}$. Relative dielectric constant of water $\varepsilon= 81$, $T = 310~\mathrm{K}$. Calculate $\lambda$ in nm and compare your answer to the thickness of the cell membrane $d\approx 5~ \mathrm{nm}$.}{0.30}

Since $\text{mmol}/\text{L} = \text{mol}/\text{m}^3$, to convert the molar concentration to normal it is necessary to multiply by Avogadro’s constant $N_A$. An ionized salt solution consists of two types of ions of equal concentration, for which $q_i^2=e^2$. Thus: \[\lambda=\sqrt{\dfrac{\varepsilon\varepsilon_0 k_BT}{2N_Ace^2}}\approx0.47~\mathrm{nm},\]which is much less than the thickness of the cell membrane.

\par\smallskip\noindent\textbf{Answer:} \[\lambda\approx0.47~\mathrm{nm}\ll d\]
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \[\sigma=-\varepsilon \varepsilon_0 E_0\]
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \[V^\mathrm{Nernst}_{\mathrm{Na^+}}=\dfrac{k_\mathrm BT} {e}\ln\dfrac{c^0_{\mathrm{Na^+}}}{c_{\mathrm{Na^+}}}\]
\end{solcard}
...
In equilibrium, the concentrations obey the Boltzmann distribution:\[c_{\mathrm{Na^+}}=c_{\mathrm{Na^+}}^0\exp\left[-\dfrac{eV}{k_BT}\right],\quad c_{\mathrm{K^+}}=c_{\mathrm{K^+}}^0\exp\left[-\dfrac{eV}{k_BT}\right],\quad c_{\mathrm{Cl^-}}=c_{\mathrm{Cl^-}}^0\exp\left[\dfrac{eV}{k_BT}\right].\]The condition for equilibrium of the solution inside the cell is the electrical neutrality of the solution:\[c_{\mathrm{Na^+}}+c_{\mathrm{K^+}}-c_{\mathrm{Cl^-}}+\rho_\mathrm{macro}/N_Ae=0.\]For the convenience of further recording, we introduce the value $\xi\equiv \exp[-eV/k_BT]$. Then numerically: \[460\xi-560\xi^{-1}-400=0\\23\xi^2-20\xi-28=0\\\xi\approx1.62\implies\\ \\V=-\dfrac{k_BT}{e}\ln1.62\approx-12.9 \mathrm{mV}\\c_{\mathrm{Na^+}}=440\cdot1.62~\text{mmol}/\text{L}=713 \mathrm{mmol}/\mathrm{L}\\c_{\mathrm{K^+}}=20 0\cdot1.62~\text{mmol}/\text{L}=32 \text{mmol}/\text{L}  \\c_{\mathrm{Cl^-}}=560/1.62~\text{mmol}/\text{L}=346 \text{mmol}/\text{L}\]

\par\smallskip\noindent\textbf{Answer:} \[V=-12.9~\mathrm{mV},\\c_{\mathrm{K^+}}=32~\mathrm{mmol}/\mathrm{L}, \quad c_{\mathrm{Na^+}}=713~\mathrm{mmol}/\mathrm{L}, \quad c_{\mathrm{Cl^-}}=346~\mathrm{mmol}/\mathrm{L}\]
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \[V=-70.0~\mathrm{mV},\\c_{\mathrm{K^+}}=568~\mathrm{mmol}/\mathrm{L}, \quad c_{\mathrm{Na^+}}=36~\mathrm{mmol}/\mathrm{L}, \quad c_{\mathrm{Cl^-}}=204~\mathrm{mmol}/\mathrm{L}\]
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \[N_\text{ATP}=1\]
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \[\alpha=\dfrac{\pi a^2\sigma}{C},\quad\beta=\dfrac{2\pi ag_\mathrm{tot}}{C}\]
\end{solcard}

\begin{solcard}{C3}{Let, as a result of excitation of the neuron, the distribution of the dynamic membrane potential is equal to to \[v(x,0)=v_0\exp\left[-\dfrac{x^2}{b^2}\right].\]Find an expression for $v(x,t)$ at $t > 0$. Note: it is convenient to look for a solution in the form \[v(x,t)=\dfrac{v_0b}{\sqrt{f(t)}}\exp\left[-\dfrac{x^2}{f(t)}\right]\exp\left[-\dfrac{t}{\tau}\right],\], where $f(t)$ is a function linear in $t$, $\tau$ is some constant.}{1.00}
...
\par\smallskip\noindent\textbf{Answer:} \[v(x,t) = \dfrac{v_0b}{\sqrt{b^2+4\pi a^2\sigma t/C}}\exp\left[-\dfrac{x^2}{b^2+4\pi a^2\sigma t/C}-\dfrac{2\pi ag_{\mathrm{tot}}t}{C}\right]\]
\end{solcard}

\begin{solcard}{C4}{For the neuron under consideration, $a=20~\mathrm{\mu m}$, $g_\mathrm{tot}=5~\mathrm{\Omega}^{-1}\cdot \mathrm{m}^{-2}$, thickness and dielectric constant of the cell membrane $d=5~\mathrm{nm}$ and $\varepsilon=7$, conductivity $\sigma=3~\mathrm{\Omega}^{-1}\cdot \mathrm{m}^{-1}$. Estimate the distance and time scales over which a neural signal propagates.}{0.40}
...
\par\smallskip\noindent\textbf{Answer:} \[\tau\sim 2.5 ~\mathrm{ms},\quad x\sim5~\mathrm{mm}\]
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \[v_{1,2}=\dfrac{H}{2}\left[1\mp\sqrt{1-\dfrac{4g_\mathrm{tot}}{BH^2}}\right]\]
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \[v_\mathrm c=v_1\]
\end{solcard}
