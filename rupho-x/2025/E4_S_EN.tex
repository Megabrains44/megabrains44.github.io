...
\par\smallskip\noindent\textbf{Answer:} \[h = 24.9~cm \quad \alpha = 18.8^\circ\]
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} $L,~cm$ $l_1,~cm$ $l_2,~cm$ $l_3,~cm$ $\langle l \rangle, cm$ 10.0 7.1 7.1 7.7 7.8 20.0 12.7 12.9 12.9 13.3 30.0 16.1 16.4 16.7 16.9 40.0 20.0 20.0 20.5 20.7 60.0 24.7 25.2 25.4 25.6
\begin{figure}[H]
  \centering
...
\par\smallskip\noindent\textbf{Answer:} \[ \gamma = 1 - \frac{9k_\textbf{A4}}{20 h \sin \alpha} = 0.36\]
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} $L,~cm$ $\langle l' \rangle,~cm$ 10.0 25.4 20.0 26.8 30.0 30.3 40.0 30.5 60.0 34.0
\begin{figure}[H]
  \centering
...
\begin{solcard}{B2}{Obtain an expression for the dependence $E_{kin}(l)$.}{0.30}

From the kinematics described above, we remove the rotational motion \[ E_{kin} = \frac{Mu^2}{2}= \frac{Mgl^2}{4h} \]

\par\smallskip\noindent\textbf{Answer:} \[ E_{kin} = \frac{Mgl^2}{4h} \]
\end{solcard}

\begin{solcard}{B3}{Using the results of steps A2 and B1, calculate the addition $\Delta E_{kin}$ to the kinetic energy of the projectile translational motion obtained during interaction with the gun, for each experimental value. Calculate the energy using the formula obtained in the previous paragraph, discarding constant factors in front of the function of $l$.}{0.40}

\[ \Delta E_{kin} = l'^2 - l^2 \]

\par\smallskip\noindent\textbf{Answer:} $L,~cm$ $\Delta E_{kin},~cm^2$ $\Delta E_{kin},~\mathrm{mJ}$ 10.0 584 2.36 20.0 534 2.16 30.0 627 2.53 40.0 497 2.00 60.0 492 1.99
\end{solcard}

\begin{solcard}{B4}{Plot a graph of $\Delta E_{kin}$ versus $l$ in coordinates in which it is linear.}{0.40}
...
\par\smallskip\noindent\textbf{Answer:} \[\beta = 0.15, \quad \Delta E_{kin}=580~cm^2=2.3~\mathrm{mJ}\]
\end{solcard}

\begin{solcard}{C1}{Using a Hall sensor, measure the magnet field for 5 values of $s$.}{0.30}
\par\smallskip\noindent\textbf{Answer:} $s,~cm$ $V+2,6~\mathrm{mV},~\mathrm{mV}$ 2.5 138.8 3.5 62.7 4.5 26.8 5.5 11.5 6.5 6.2 7.5 3.4
\begin{figure}[H]
  \centering
...
\par\smallskip\noindent\textbf{Answer:} $$[m] = A \cdot \mathrm{m}^2$$$$[M]  = A \cdot \mathrm{m}^{-1}$$
\end{solcard}
...
Magnetic dipole field: $$\vec{B} = \frac{\mu_0}{4\pi}\left(\cfrac{3(\vec{m}\vec{r})\vec{r}}{r^5} - \cfrac{\vec{m}}{r^3}\right)$$in projection on the axis directed along the dipole moment $\vec{m}$: $$B(s) = \frac{\mu_0}{4\pi}\cdot \cfrac{2m}{s^3}$$

\par\smallskip\noindent\textbf{Answer:} $$B(s) = \frac{\mu_0}{4\pi}\cdot \cfrac{2m}{s^3}$$Direction - along magnetic moment $\vec{m}$.
\end{solcard}

\begin{solcard}{C4}{Using the results of the previous two paragraphs, construct a graph of the dependence $B(s)$ in coordinates in which it will be linear. Determine the magnetization value of the magnet $M$.}{0.60}
...
$$B(s) = \frac{\mu_0}{4\pi}\cdot \left( \cfrac{2m'}{s^3} + \cfrac{2m}{(s+x)^3} \right)$$At the same time the induced field gives a linearization $V^{-1/3}$ from $s$.

\par\smallskip\noindent\textbf{Answer:} $x,~cm$ $m',~\mathrm{A} \cdot \mathrm{m}^2$ 1.43 0.030 1.65 0.021 2.19 0.0077 2.59 0.0057 2.92 0.0031
\end{solcard}

\begin{solcard}{C7}{Write down the boundary conditions for the vectors $\vec{B}$ and $\vec{H}$ at the steel-air interface. Using the boundary conditions, obtain the relationship between the angles that form the magnetic field induction vector $\vec{B}$ with the normal to the surface in both media. The resulting relationship may contain the specified angles and $\mu_\text{steel}$. Obtain an expression for the angle between the magnetic field induction vector and the normal in air in the limiting transition $\mu_\text{steel}\gg 1$.}{0.40}

1 - index of quantities in steel 2 - index of quantities in air As a consequence of the Gauss theorem for the Magnetic field, the normal component of the magnetic field $\vec{B}$ is preserved: $$B_{n1} = B_{n2}$$in the system there are no surface magnetic currents, so the tangential component $\vec{H}$ be conserved: $$H_{\tau 1} = H_{\tau 2}$$Use relation $\vec{B} = \mu \mu_0 \vec{H}$: $$B_{\tau 1} / \mu = B_{\tau 2}$$$$B_{n1} \tan{\theta_1} / \mu = B_{n2} \tan{\theta_2}$$$$\tan{\theta_1} / \mu = \tan{\theta_2}$$For $\mu >> 1$, obtain $\theta_2 = 0$.

\par\smallskip\noindent\textbf{Answer:} \[\begin{cases} B_{n1} = B_{n2} \\ H_{\tau 1} = H_{\tau 2} \end{cases} \] \[ \tan{\theta_\text{steel}} = \tan{\theta_\text{air}}\cdot \mu_\text{steel}\] \[ \quad\theta_2 = 0 \]
\end{solcard}

\begin{solcard}{C8}{What type of substance does $\varepsilon \gg 1$ correspond to in electrostatics?}{0.10}

The boundary conditions in a conductor in electrostatics are similar before the replacement: $$\vec{H} \Leftrightarrow \vec{E}$$$$\vec{B} \Leftrightarrow  \vec{D}$$ The limit $\varepsilon \to \infty$ in electrostatics is a transition to the conductor.

\par\smallskip\noindent\textbf{Answer:} Conductor
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} At the center of the sphere there is a magnetic charge $q_m \cdot R/y$ At a distance $R/y$ from the center of the sphere $-q_m \cdot R/y$
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} $$m' = \cfrac{2mR^3}{x^3} $$
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} $$t_1 = 1/4$$$$t_2 = 1/32$$
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} $$m_1 = m_2 = 8/31 m$$
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} $$B(x) = \cfrac{\mu_0}{2\pi}\left( \frac{m}{x^3} + \frac{m_4}{(x+2R)^3} + \frac{m_3}{(x + 4R)^3} \right)$$
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} $$B(x) = \cfrac{\mu_0}{2\pi}\left( \frac{m_2}{(x+2R)^3} + \frac{m}{x^3} + \frac{m_1}{(x - 2R)^3} \right)$$
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \[ W_1 = 2.68 \cdot 10^{-3} \cfrac{\mu_0 m^2}{R^3}\]
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \[ W_2 =  - 3.83 \cdot 10^{-4}  \cdot \cfrac{\mu_0 m^2}{R^3}\]
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \[E_{\max}=2.38~\mathrm{mJ}\]
\end{solcard}
