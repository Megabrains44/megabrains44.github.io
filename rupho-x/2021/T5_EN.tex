...
\problembox{C4}{Under the described new assumptions, repeat calculations similar to point C2 and obtain expressions for $\cfrac{h_2}{h_1}$ and for the ratio $\cfrac{E_\text{кин}}{E_\text{пот}}$ of the kinetic energy of the flying link to its initial potential energy (measured from the floor level).}{0.60}

\problembox{C5}{What condition must $\alpha$ and $\beta$ satisfy so that the law of conservation of energy is not violated? At what values of $\alpha_0$ and $\beta_0$ is the maximum permissible height of the “fountain” achieved? What are the corresponding ratios $\left( \cfrac{h_2}{h_1} \right)_{max}$ and $\left( \cfrac{E_\text{кин}}{E_\text{пот}} \right) _{max}$ equal to?}{0.60}
...
\problembox{E2}{Assuming that the chain is “pins” connected to each other (i.e., the links are thin cylinders), calculate $\alpha_\text{шт.}$ - the numerical value of the coefficient introduced earlier by the relation $R=\alpha \lambda v^2$. Take $\beta \approx 0$ and find $\left( \cfrac{h_2}{h_1} \right)_\text{шт.}$.}{0.30}

\problembox{E3}{Now considering the links to be thin hoops, calculate $\alpha_\text{обр.}$. Take $\beta \approx 0$ and find $\left( \cfrac{h_2}{h_1} \right)_\text{обр.}$.}{0.30}
