...
A complex molecule will be characterized not by one frequency $\omega_0$, but by a whole spectrum of natural frequencies. Knowledge of this spectrum allows us to unambiguously identify the substance with which we are dealing. This is the main idea of ​​SERS. Let us study the behavior of a molecule in an external electric field $\vec{E}_0\cos\omega t$. For simplicity, we will assume that the atoms are not charged, i.e. in the absence of an external field, the molecule does not have a dipole moment. However, in an external field, like a metal ball, the molecule is polarized, i.e. acquires a dipole moment $$\textbackslash\{\}vec\{d\}=\textbackslash\{\}varepsilon_0\textbackslash\{\}alpha\textbackslash\{\}vec\{E\}_0\textbackslash\{\}cos\textbackslash\{\}omega t $$ где $\alpha$ – поляризуемость молекулы. Здесь мы считаем, что направление индуцированного дипольного момента $\vec{d}$ совпадает с направлением внешнего поля

Механические колебания, изученные в предыдущем пункте, никогда не прекращаются, атомы в молекуле все время дрожат. Это означает, что величина отклонения расстояния между атомами от положения равновесия $x$ изменяется со временем, $x=x_0\cos\omega_0t$. В свою очередь, способность молекулы поляризоваться зависит от взаимного расположения атомов, то есть $\alp ha=\alpha(x)$ (cm. рис.2).
...
\problembox{C3}{Calculate the dipole moment of a diatomic molecule in an external field $\vec{E}_0\cos\omega t$. Present the answer as: $$\textbackslash\{\}vec\{d\}=\textbackslash\{\}sum_i\textbackslash\{\}vec\{d\}_i\textbackslash\{\}cos\textbackslash\{\}omega_it $$}{0.20}
...
\problembox{D4}{Using the results of the second part of the problem, estimate the size $R$ of the metal particles used in the experiment if the wavelength of the incident radiation is $\lambda=785~\text{нм}$.}{0.50}
