...
Within the framework of the problem, consider all media to be nonmagnetic ($\mu\approx 1$), and all waves to be monochromatic. Also consider that there are no external currents or free charges at the interface. To describe the electric field strengths, let's move on to complex amplitudes. Next, take all directions of electric field strengths as those shown in the figure, and write down the magnitudes of electric field strengths in the following form: $$E_\{пад\}=\textbackslash\{\}operatorname\{Re\}\textbackslash\{\}left(E_0e^\{i(\textbackslash\{\}omega t-\textbackslash\{\}vec\{k\}\textbackslash\{\}vec\{r\})\}\textbackslash\{\}right)\textbackslash\{\}qquad E_\{отр\}=\textbackslash\{\}operatorname\{Re\}\textbackslash\{\}left(E_re^\{i(\textbackslash\{\}omega t-\textbackslash\{\}vec\{k\}_r\textbackslash\{\}vec\{r\})\}\textbackslash\{\}right)\textbackslash\{\}qquad E_\{прош\}=\textbackslash\{\}operatorname\{Re\}\textbackslash\{\}left(E_te^\{i(\textbackslash\{\}omega t-\textbackslash\{\}vec\{k\}_t\textbackslash\{\}vec\{r\})\}\textbackslash\{\}right)\{.\} $$Здесь $E_0$, $E_r$ и $E_t$ – комплексные амплитуды напряжённостей электрический полей падающей, отражённой и прошедшей волн соответственно в точке $O$, от которой отсчитывается радиус-вектор $\vec{r}$. Введём также комплексные коэффициенты отражения и прохождения по амплитуде $r$ и $t$ соответственно. Они определяются следующими выражениями: $$r=\textbackslash\{\}cfrac\{E_r\}\{E_0\}\textbackslash\{\}qquad t=\textbackslash\{\}cfrac\{E_t\}\{E_0\}\{.\} $$

\problembox{A1}{Let a plane electromagnetic wave propagate in a substance with a refractive index $n$ with an oscillation amplitude $E$ of the electric field strength. Express the amplitude of fluctuations in the magnetic field strength $H$ in this wave in terms of $E$, $n$, as well as the magnetic and electric constants $\mu_0$ and $\varepsilon_0$, respectively.}{0.30}
...
\problembox{A2}{What 4 boundary conditions for the electric and magnetic fields are satisfied at the interface? Write them down on the answer sheets in a form convenient for you.}{0.40}

\problembox{A3}{All boundary conditions that are met at the interface between two media must be satisfied at each point at an arbitrary point in time. Based on this, prove the following equalities: $$\textbackslash\{\}omega_r=\textbackslash\{\}omega_t=\textbackslash\{\}omega\textbackslash\{\}qquad \textbackslash\{\}theta_r=\textbackslash\{\}theta\textbackslash\{\}qquad n_2\textbackslash\{\}sin\textbackslash\{\}theta_t=n_1\textbackslash\{\}sin\textbackslash\{\}theta\{.\} $$}{0.40}

\problembox{A4}{Derive an expression for the complex reflection and transmission coefficients $r_s$ and $t_s$, respectively, in the case of a $s$-polarized wave. Express your answers in terms of $n_1$, $n_2$, $\theta$ and $\theta_t$. The expressions for $r_s$ and $t_s$ are called Fresnel formulas for the $s$-polarized wave.}{0.50}

\problembox{A5}{Derive an expression for the coefficients $r_p$ and $t_p$ in the case of a $p$-polarized wave. Express your answers in terms of $n_1$, $n_2$, $\theta$ and $\theta_t$. The expressions for $r_p$ and $t_p$ are called Fresnel formulas for the $p$-polarized wave.}{0.50}
...
\problembox{A6}{Determine the value of $\cos\theta_t$. Express your answer in terms of $n_1$, $n_2$ and $\theta$.}{0.30}

\problembox{A7}{The expressions for $r_s$ and $r_p$ can be represented as follows: $$r_s=A_se^\{i\textbackslash\{\}delta_p\}\textbackslash\{\}qquad r_p=A_pe^\{i\textbackslash\{\}delta_p\}\{.\} $$Определите $A_s$, $A_p$, $\delta_s$ и $\delta_p$. Ответы выразите через $n_1$, $n_2$ и $\theta$.}{0.40}

Let $\delta=\delta_p-\delta_s$.

\problembox{A8}{Define $\delta$. Express your answer as one inverse trigonometric function in terms of $\theta$ and $n_{21}=n_2/n_1$.}{0.40}
...
\problembox{B1}{At what values ​​of the acute angle $\alpha$ of the Fresnel rhombus will the electromagnetic wave experience total reflection from the faces of the Fresnel rhombus $BC$ and $AD$? Express your answer in terms of $n$ and calculate it in degrees to four significant figures.}{0.10}

\problembox{B2}{At what values ​​of the angle $\alpha=\alpha'$ can we obtain circular polarization from linear polarization using the Fresnel rhombus? Express your answers in terms of $n$ and calculate them in degrees to four significant figures.}{0.40}

\problembox{B3}{Let a beam of light fall on the Fresnel rhombus over the entire area of ​​the face $AB$. Angle $\alpha=\alpha'$. At what values ​​of $BC/AB$ does the Fresnel rhombus reverse the entire beam of rays (all rays entering the Fresnel rhombus through the face $AB$ experience two total reflections)? Calculate your answer to three significant figures.}{0.30}
...
\problembox{B4}{At what values ​​of the angle $\alpha$ of the Mooney rhombus will the electromagnetic wave experience total reflection from the faces of the Fresnel rhombus $BC$ and $CD$? Express your answer in terms of $n$ and calculate it in degrees to four significant figures.}{0.10}

\problembox{B5}{At what values ​​of the angle $\alpha=\alpha'$ can we obtain circular polarization from linear polarization using the Mooney rhombus? Express your answers in terms of $n$ and calculate them in degrees to four significant figures.}{0.30}

\problembox{B6}{Let the angle $\alpha=\alpha'$. Determine the maximum possible value $\gamma$ of the ratio of the cross-sectional area of ​​the beam incident on the face $AB$ to the area of ​​the face $AB$, if the Mooney rhombus reverses the entire beam of rays (all rays entering the Mooney rhombus through the face $AB$ experience two total reflections). Calculate your answer to three significant figures.}{0.30}
...
\problembox{C1}{Show the validity of the following relations: $$r'_1=-r_1\textbackslash\{\}qquad 1-r^2_1=t_1t'_1\{.\} $$}{0.30}
...
\problembox{C3}{Reasoning similarly to point C2, express $E'$ and $E_t$ in terms of $E$, $r_2$ and $t'_2$, $n_1$, $n_2$, $n$, $k$, $d$ and $\theta$. Note that the complex amplitudes of the electric fields at point $O'$ are out of phase with respect to the corresponding complex amplitudes at point $O$.}{0.60}

\problembox{C4}{Using the results from steps C1-C3, determine the reflectance $r=E_r/E_1$. Express your answer in terms of $r_1$, $r_2$, $n_1$, $n$, $k$, $d$ and $\theta$.}{0.60}
...
\problembox{C5}{At what value of the plate thickness $d$ is it possible for the reflection coefficient to vanish? Express your answer in terms of $\lambda_1$, $n_1$ and $n$.}{0.70}

\problembox{C6}{At what values ​​of the refractive index of the plate $n$ is it possible for the reflection coefficient to vanish? Express your answer in terms of $n_1$ and $n_2$.}{0.80}
...
\problembox{D1}{What is the refractive index $n'$ of the antireflection coating? Express your answer in terms of $n$.}{0.20}

\problembox{D2}{What is the focal length of the lens $f$? Express your answer in terms of $R$ and $n$}{0.50}

\problembox{D3}{Determine the force $F$ acting on the lens. Express your answer in terms of $I_0$, $r$, $R$, $n$ and $c$.}{1.00}

\vspace{1em}
\noindent\textit{Source:} \url{https://pho.rs/p/3868}
