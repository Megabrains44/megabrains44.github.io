...
\begin{table}[h]
\centering
\caption{Rocket mass as a function of elapsed time (u/c = 0.1)}
\label{tab:rocket_mass}
\begin{tabular}{cc}
\hline
$x = \tau/\tau_0$ & $y = m/M$ \\
\hline
...
v(\tau) = v_0(\tau) + \delta v(\tau)
\end{equation}

\begin{equation}
m(\tau) = m_0(\tau) + \delta m(\tau)
\end{equation}

where $v_0(\tau)$ and $m_0(\tau)$ are the zeroth-order solutions 
and $\delta v(\tau)$ and $\delta m(\tau)$ are small corrections such that 
$\delta v(\tau) \ll v_0(\tau)$ and $\delta m(\tau) \ll m_0(\tau)$.

Express the corrections in terms of $M$, $\rho$, $S$, $c$, $u$, and $a$.
