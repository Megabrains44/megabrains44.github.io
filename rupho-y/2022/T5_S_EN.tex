...
\begin{solcard}{A1}{Find the value of the radius of the circular orbit $R_\text{ДПС}$ at which the mechanical stress in the area of ​​contact of the objects is equal to zero. The quantity $R_\text{ДПС}$ is the Roche limit $R_{Roche}$ for a binary planetary system}{1.50}
...
\par\smallskip\noindent\textbf{Answer:} \textbackslash{}textbf{Answer:} $$R_{ДПС}=r\sqrt[3]{\displaystyle\frac{12M}{m}} $$
\end{solcard}

\begin{solcard}{B1}{Obtain an exact expression for the total potential $\varphi_\text{внеш}$ in the field of the centrifugal force and the gravitational field of the planet. Express your answer in terms of $x$, $y$, $z$, $G$, $\rho_M$, $R_0$ and $R$.}{0.70}
...
\par\smallskip\noindent\textbf{Answer:} \textbackslash{}textbf{Answer:} $$\varphi=\frac{2\pi R^3_0\rho_MG}{R}-\cfrac{4\pi R^3_0\rho_MG}{3R}\left(\cfrac{R}{\sqrt{(R+x)^2+y^2+z^2}}+\cfrac{(R+x)^2+y^2}{2R^2}\right) $$
\end{solcard}

\begin{solcard}{B2}{In the $x,y,z\ll{R}$ approximation, the expression for the $\varphi$ potential can be represented as follows: $$\varphi_{\text{внеш}}=A_1x^2+B_1y^2+C_1z^2 $$ Find coefficientы $A_1$, $B_1$ and $C_1$. Answerы выtimesandте via/through $R$, $G$, $R_0$ and $\rho_M$.}{0.80}
...
\par\smallskip\noindent\textbf{Answer:} \textbackslash{}textbf{Answer:} $$A=-\cfrac{2\pi R^3_0\rho_MG}{R^3} \quad B=0 \quad C=\cfrac{2\pi R^3_0\rho_MG}{3R^3} $$
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \textbackslash{}textbf{Answer:} $$\varphi_2=\varphi_0+(\varphi_1-\varphi_0)\gamma^2 $$
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \textbackslash{}textbf{Answer:} $$\varphi=\varphi_0-\frac{\partial g_x}{\partial x}\frac{x^2}{2}-\frac{\partial g_\perp}{\partial r_\perp}\frac{(y^2+z^2)}{2} $$
\begin{figure}[H]
  \centering
...
\begin{solcard}{C4}{Express $B_2$ in terms of $A_2$, $G$ and $\rho$. $\textit{Примечание:}~$ use Gauss's theorem in differential form for the gravitational field: $$\displaystyle\frac{\partial^2{\varphi}}{\partial{x}^2}+\displaystyle\frac{\partial^2{\varphi}}{\partial{y}^2}+\displaystyle\frac{\partial^2{\varphi}}{\partial{z}^2}=4\pi\rho G $$}{0.30}
...
\par\smallskip\noindent\textbf{Answer:} \textbackslash{}textbf{Answer:} $$B_2=\pi G\rho-\frac{A_2}{2} $$
\end{solcard}

\begin{solcard}{C5}{The potential $\varphi$ on the surface of the ellipsoid can be represented as a function of only the coordinate $x$ in the following form: $$\varphi=C_2+x^2D_2 $$ Express $C_2$ and $D_2$ via/through $\varphi_0$, $A_2$, $G$, $\rho$, $a$, $x$ and эксцентриситет $e$.}{0.30}
...
\par\smallskip\noindent\textbf{Answer:} \textbackslash{}textbf{Answer:} $$C_2=\varphi_0+\left(\pi\rho G-\cfrac{A_2}{2}\right)a^2(1-e^2) \quad D_2=\cfrac{A_2(3-e^2)}{2}-\pi\rho G(1-e^2) $$
\end{solcard}

\begin{solcard}{C6}{Let us solve an auxiliary problem: a mass with surface density $\sigma$ is uniformly distributed over the surface of a disk of radius $R$. Find the potential of the disk $\varphi_\text{д}$ on the axis at a distance $x$ from its center. Express your answer using $\sigma$, $G$, $R$ and $x$.}{0.50}

Let's split the disk into rings: $$\varphi_\text{д}=-\pi\sigma G\int\limits_0^R\frac{2rdr}{\sqrt{r^2+x^2}} $$

\par\smallskip\noindent\textbf{Answer:} \textbackslash{}textbf{Answer:} $$\varphi_\text{д}=-2\pi\sigma G\left(\sqrt{R^2+x^2}-x\right) $$
\end{solcard}

\begin{solcard}{C7}{Find the potential $\varphi_0$ at the center of the ellipsoid. Express your answer in terms of $G$, $\rho$, $a$ and $e$. $\textit{Примечание:}$ you may need the following integral: $$\displaystyle\int\sqrt{1+x^2}dx=\cfrac{x\sqrt{1+x^2}+\ln\left(x+\sqrt{1+x^2}\right)}{2}+C $$}{1.00}
...
\par\smallskip\noindent\textbf{Answer:} \textbackslash{}textbf{Answer:} $$\varphi_0=-\frac{2\pi\rho Ga^2(1-e^2)}{e}\ln\sqrt{\cfrac{1+e}{1-e}} $$
\end{solcard}

\begin{solcard}{C8}{Find the potential of the ellipsoid $\varphi_a$ at the point with coordinates $(x, y, z)=(a, 0, 0)$. Express your answer using $G$, $\rho$, $a$ and $e$. $\textit{Примечание:}$ you may need the following integral: $$\displaystyle\int\sqrt{x^2-1}dx=\cfrac{x\sqrt{x^2-1}-\ln\left(x+\sqrt{x^2-1}\right)}{2}+C $$}{1.00}
...
\par\smallskip\noindent\textbf{Answer:} \textbackslash{}textbf{Answer:} $$\varphi_a=-2\pi\rho Ga^2\left(\frac{(1-e^2)}{e^2}-\frac{(1-e^2)^2}{e^3}\ln\sqrt{\frac{1+e}{1-e}}\right) $$
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \textbackslash{}textbf{Answer:} $$A_2=2\pi\rho G\left(\frac{1-e^2}{e^3}\ln\sqrt{\frac{1+e}{1-e}}-\frac{(1-e^2)}{e^2}\right) $$
\end{solcard}
...
\par\smallskip\noindent\textbf{Answer:} \textbackslash{}textbf{Answer:} $$e=0,86\pm{0,02} $$
\begin{figure}[H]
  \centering
...
\par\smallskip\noindent\textbf{Answer:} \textbackslash{}textbf{Answer:} $$R_{Roche}\approx{2,43R_0\sqrt[3]{\frac{\rho_M}{\rho_m}}} $$
\end{solcard}
