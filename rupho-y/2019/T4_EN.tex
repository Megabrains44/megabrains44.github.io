...
Figure 2. Photocycle of channel rhodopsin,
three-state
...
\problembox{A2}{Please indicate which ones belong to which channel? The height of the step is the same everywhere and is equal to $I=1.4~\text{пА}$.}{0.50}
...
The relationship between the diffusion coefficient $D$, mobility $\mu$ and temperature $T$ was discovered when studying Brownian motion of particles independently by Einstein and Smoluchowski. Einstein-Smoluchowski relation: $$D=\textbackslash\{\}mu kT, $$где $k$ – Boltzmann constant.

\problembox{B2}{Find at what voltage $U_\text{rev}$ on the cell membrane (membrane potential) the flux through the open potassium channel becomes zero at temperature $T=20^{\circ}\text{C}$. Express your answer in terms of $T$, $c_\text{out}=4~\text{mmоль}/\text{л}$ and $c_\text{in}=155~\text{mmоль}/\text{л}$ – potassium concentrations outside and inside the cell, and fundamental physical constants. Note: Membrane potential is considered positive if the plus is on the inner surface of the cell membrane.}{2.00}
